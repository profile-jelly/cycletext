...
print(torch.cuda.current_device()) # 当前默认 GPU
...
## Model
parser.add_argument("--t5_tokenizer", default=r"G:\models\t5-base", type=str,
                    help="Tokenizer for T5 models")
parser.add_argument("--data2text_model", default=r"G:\abnormal\cycletext\models\webnlg_t5_data2text_100", type=str,
                    help="Local or Huggingface transformer's path to the data2text model")
parser.add_argument("--text2data_model", default=r"G:\abnormal\cycletext\models\webnlg_t5_text2data_100", type=str,
...
parser.add_argument("--start_epoch", default=9, type=int,
                    help="Epoch number to start training from (useful for resuming training)")
...
if args.do_eval:
    if args.text2data_validation_file != None:
        ...
        text2triplets_val = load_dataset(
            'csv',
            data_files={'dev': args.text2data_validation_file},
            delimiter='\t',
            column_names=['source', 'target']   # 指定列名
        )
    ...
    if args.data2text_validation_file != None:
        ...
        triplets2text_val = load_dataset(
            'csv',
            data_files={'dev': args.text2data_validation_file},
            delimiter='\t',
            column_names=['source', 'target']  # 指定列名
...
if args.do_test:    
    if args.text2data_test_file != None:
        ...
        text2triplets_test=load_dataset(
            'csv',
            data_files={'test': args.text2data_validation_file},
            delimiter='\t',
            column_names=['source', 'target']  # 指定列名
        )
    ...
    if args.data2text_test_file != None:
        ...
        triplets2text_test=load_dataset(
            'csv',
            data_files={'test': args.text2data_validation_file},
            delimiter='\t',
            column_names=['source', 'target']  # 指定列名
...
metric_bertscore = load("bertscore",device=args.bertscore_gpu_device)
print ("bertscore device = ", args.bertscore_gpu_device)
